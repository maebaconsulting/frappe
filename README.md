...
deux conteneurs éphémères, `configurator` et `create-site`, dont les autres services
dépendent par la condition `service_completed_successfully`. En mode Swarm, ces
...
Le fichier Compose ne déclare pas de service `migrator`. La migration de la base après
un changement d'image est donc une opération explicite. Sans elle, le site peut se
retrouver incohérent ou refuser de démarrer.

Séquence complète :

1. Relancer le workflow depuis l'onglet Actions.
2. Redéployer le service dans Dokploy. Les services utilisent `pull_policy: always`,
   la nouvelle image est donc tirée même à tag identique.
3. Lancer la migration sur le serveur :
...
Pour revenir en arrière, renseigner un tag horodaté dans `ERP_IMAGE`, par exemple
`ghcr.io/maebaconsulting/frappe:version-16-20260803-141205`, puis redéployer. Attention :
un retour en arrière d'image ne défait pas une migration de base déjà appliquée. Une
sauvegarde préalable reste indispensable.
